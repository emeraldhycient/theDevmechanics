	...
	return (
		<section className={`py-2 flex flex-col gap-y-8 lg:px-14 px-3`}>
			{Services.map((service, index) => (
				<div
					className={`flex gap-x-36 ${index % 2 == 0 ? "lg:flex-row flex-col" : "lg:flex-row-reverse flex-col"} py-3 gap-y-10`}
					key={index}>
					<Image
						src={service.image}
						alt="Service Image"
					...
					<section className={`flex-1`}>
						<header
							className={`min-h-[9rem] lg:px-5 px-2 py-2 gap-x-6 flex items-center`}>
							<h1
							...
							<div
								className={`h-[4rem] w-[1px] bg-[#9743FF33]`}></div>
							...
							<section>
								...
								<h2
									className={`font-bold lg:text-[30px] text-2xl`}>
									{service.title}
								</h2>
							</section>
						</header>
						...
						<div>
							<p
								className={`text-[#676767] lg:text-[15px] text-sm`}>
								{service.text}
							</p>
