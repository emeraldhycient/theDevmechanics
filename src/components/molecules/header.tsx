...
const Header = (): React.ReactNode => {
  ...
  return (
    <header className={`flex relative duration-200 justify-between py-4 lg:px-16 md:px-10 px-3 items-center sticky top-0 left-0 text-sm z-100 bg-white`}>
      ...
      <button className={`lg:hidden md:hidden block text-3xl`} onClick={() => { setMobileNavOpen(!mobileNavOpen) }}>
        <IoReorderTwoOutline />
      </button>

      <div onClick={() => setMobileNavOpen(!mobileNavOpen)} className={`bg-white w-full overflow-hidden ${!mobileNavOpen ? "-translate-[100%] py-0 h-0" : "translate-[120%] py-4 h-[100vh]"} absolute top-14 left-0 duration-300 px-4`}>
        <nav>
          <ul className={`flex flex-col gap-y-4 w-full [&_li]:text-xl`}>
            {Links.map((link, index) => (
              <li key={index}>
                <Link href={link.href}>
                  <p className={`py-2 flex w-full items-center justify-between`}>
                    {link.name}
